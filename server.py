...
def _filter_blendshapes(raw: Dict[str, float]) -> Dict[str, float]:
    ...
    for k in BLENDSHAPE_KEYS:
        v = raw.get(k)
        if v is not None and v > 0.01:   # skip noise
            out[k] = round(v, 3)
    return out
...
class EmotionLogRequest(BaseModel):
    ...
    blendshapes: Dict[str, float] = {}     # ← new: from MediaPipe blendshapes
...
async def emotion_from_logs(req: EmotionLogRequest):
    ...
    logging.info(
        ...
        f"blendshapes={len(req.blendshapes)}"
    )
    ...
    resp = {
        "trust": trust, "safety": safety, "risk": risk,
        "emotion": emotion,
    ...
    logging.info(
        ...
        f"conf={confidence:.2f} trust={trust:.2f} safety={safety:.2f} risk={risk:.2f}"
    )
    return resp
