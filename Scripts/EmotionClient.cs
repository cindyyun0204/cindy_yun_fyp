...
// JsonUtility can't serialise Dictionary<string,float> directly.
// We store blendshapes as parallel key/value arrays and reconstruct on the
// server side — but since server.py uses pydantic Dict[str,float] we instead
// serialise to a JSON object string and embed it as a raw string field, then
// fix it up before sending.  Simplest approach: manual JSON building.
[Serializable]
public class EmotionRequest
{
    ...
    // blendshapes is injected as raw JSON after JsonUtility serialisation
}
...
public class EmotionClient : MonoBehaviour
{
    ...
    {
        "t_utc", "user_id", "condition_id", "group_id", "session_id", "driving_session", "asr_text",
    ...
    /// Called by the runner each time new blendshapes arrive from MediaPipe.
    ...
    public IEnumerator PostOnce(string utcIso, string asr, FaceFeatures features, bool drivingSession = true)
    {
        ...
        // Build JSON manually so we can embed blendshapes as a proper object
            ...
            try
            {
                ...
                if (logResponses && parsed != null)
                    Debug.Log($"[EmotionClient] Emotion={parsed.emotion} conf={parsed.confidence:0.00} " +
                              $"trust={parsed.trust:0.00} safety={parsed.safety:0.00} risk={parsed.risk:0.00}");
            }
            catch (Exception e)
            {
    ...
    {
        // Build session_id from StudyIDs so it's consistent with the CSV
        string sid = string.IsNullOrEmpty(StudyIDs.SessionTag) ? sessionId : StudyIDs.SessionTag;

        var sb = new StringBuilder();
        sb.Append('{');
        sb.Append($"\"session_id\":\"{EscJ(sid)}\",");
        ...
        string httpResult, string httpError, EmotionResponse parsed)
    {
        ...
        CsvLog.AppendRow(
            utcIso,
            StudyIDs.UserId,
            StudyIDs.ConditionId,
            StudyIDs.GroupId,
            string.IsNullOrEmpty(StudyIDs.SessionTag) ? sessionId : StudyIDs.SessionTag,
            driving ? "true" : "false",
            asr,
            ...
            httpResult,
            httpError ?? "",
            parsed != null ? parsed.trust.ToString("F6")      : "",
            parsed != null ? parsed.safety.ToString("F6")     : "",
            parsed != null ? parsed.risk.ToString("F6")       : "",
